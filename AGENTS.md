...
- Room 是交易、股票、帳戶與歷史價格的唯一資料來源；目前資料庫版本為 20。修改 entity 必須新增 migration，不可破壞既有資料。
...
- 「部分賣出列入已實現」預設關閉；關閉時不得改變既有分類與損益。開啟後首頁以移動平均成本拆分已實現／未實現，已發生現金事件歸已實現，未付應計成本歸未實現，兩者合計必須等於既有累積損益。減資彌補虧損若未退還股款，只減少股數並提高每股成本，不提前認列已實現虧損。
- 「損益與報酬排除股息收入」預設關閉；開啟後實領現金股息不納入即時與歷史損益、成本報酬率及 XIRR，但股息收入摘要、交易紀錄與股票配股股數仍維持原資料。
...
- 首次安裝或版本更新後顯示該版更新重點；一般關閉只隱藏至下次啟動，選擇不再顯示才以 DataStore 記錄目前版本，並於下次版本更新後再次提示。更新文案需使用一般使用者能理解的功能效益描述。
