...
- 台股股票清單現在保留 `Stock.exchange` 交易所分類（上市、上櫃、興櫃），Room 由 v10 migration 升至 v11；融資欄位由 v11 升至 v12、融券欄位再由 v12 升至 v13。App 啟動時會用 bundled `stocks.json` 依代號回填舊資料。`Stock.market` 仍只表示 `TW` / `US`，並維持 `code` 唯一索引。
