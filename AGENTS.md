...
- 目前包含：
  ...
  - 手續費與稅率設定
  - 即時股價快取
  - 融資功能開關（實驗階段，預設關閉）與利息計息基準（365/360 天，預設 365）

融資目前限定台股。開啟後，融資買進每筆保存融資本金、年利率與穩定批次識別碼；還款由使用者指定單一未償批次，利息採買進日開始、還款日不計的逐日單利。功能開關只控制 UI，不會刪除或改寫既有融資資料。
...
- 台股股票清單現在保留 `Stock.exchange` 交易所分類（上市、上櫃、興櫃），Room 由 v10 migration 升至 v11；融資欄位再由 v11 升至 v12。App 啟動時會用 bundled `stocks.json` 依代號回填舊資料。`Stock.market` 仍只表示 `TW` / `US`，並維持 `code` 唯一索引。
