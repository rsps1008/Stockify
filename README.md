# 韭菜記帳本（Stockify）- 您的個人股票帳本

## 授權

本專案採用 [MIT License](./LICENSE)。

簡單來說，您可以自由使用、複製、修改、合併、發布與散布這個專案的原始碼與衍生作品，但必須保留原始版權與授權聲明，且專案不提供任何保證。
...
為了重現那段美好的免費使用時光，並確保投資管理者在任何裝置上都能自由操作，我借助 Vibe-Coding、Gemini 和 GPT 的力量，從頭打造了這個全新的韭菜記帳本（Stockify）。它開源、免費，且永遠歡迎社群共同改進。

## 功能截圖

### 核心功能展示
| 01 - 主畫面 | 02 - 交易明細 | 03 - 資料管理 | 04 - 除權息查詢 |
| :---: | :---: | :---: | :---: |
| ![主畫面](./screenshots/01.png) | ![交易明細](./screenshots/02.png) | ![資料管理](./screenshots/03.png) | ![除權息](./screenshots/08.png) |

### 更多介面
| 05 - 設定 | 06 - 新增資料 | 07 -詳細1 | 08 - 詳細2 |
| :---: | :---: | :---: | :---: |
...
### [v1.0.5] - 2026-04-10
**本次更新重點：**
- **即時資料來源切換**：設定頁切換 `TWSE` / `Yahoo` 後，背景即時報價服務會立即套用，不需要重啟 App。
- **Yahoo 即時報價修正**：補上千分位數字解析，並降低同時連線數，減少 `connect timeout` 與解析失敗。
- **備援流程收斂**：每次抓價只做一次主來源與一次備援來源，不會在失敗後回頭重試主來源。
- **底部導覽修正**：從 `StockDetailScreen` 切換到底部導覽列時，不會再把詳細頁一併恢復回來。
- **進頁 log 統一**：各個 destination 進入時會統一記錄 `Enter XXXScreen`，方便追蹤頁面跳轉。
...
韭菜記帳本（Stockify）是一款使用 Kotlin 和 Jetpack Compose 開發的原生 Android 應用程式，旨在幫助您輕鬆追蹤股票投資組合與交易紀錄。專案遵循現代化的 Android 開發實踐，擁有清晰的架構和完善的依賴管理。
...
韭菜記帳本的操作介面直觀簡潔，主要功能都圍繞著「持股」與「交易」兩大核心。
...
*   從其他軟體無痛轉移您的交易紀錄。
*   將韭菜記帳本（Stockify）的資料備份或用於其他分析工具。
