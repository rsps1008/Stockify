...
interface StockDao {
    ...
    @Query("SELECT DISTINCT s.* FROM stocks s JOIN stock_transactions st ON s.id = st.stockId")
    fun getHeldStocks(): Flow<List<Stock>>

    @Query("SELECT * FROM stock_transactions WHERE stockId = :stockId ORDER BY date DESC")
    fun getTransactionsForStock(stockId: Int): Flow<List<StockTransaction>>
